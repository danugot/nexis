...
## 🧰 Direction 6: Data Quality & Human-in-the-Loop (数据质量与交互微调)
This targets the noise generated during the automated parsing and relationship extraction process, ensuring high-fidelity knowledge graphs.
- **Pending Suggestion Visualization (解决 Unnamed 节点)**: During ingestion, when the LLM suggests a new categoriztion (`TaxonomySuggestion`), it immediately writes to the Graph to link entities. Because this is unapproved, the node lacks a formal name and appears as "Unnamed" or "Submodule". We will sync the `proposedPath` to the graph and visually distinguish pending suggestions, removing confusion.
- **Suggestion Fine-Tuning (智能建议微调)**: When the system proposes a new Taxonomy path or identifies a Business Conflict, the current actions are binary (Approve/Reject). We will introduce "Fine-Tuning" API endpoints and UI hooks, allowing PMs to manually correct the LLM's suggested category path or conflict resolution before committing it to the formal structure.
